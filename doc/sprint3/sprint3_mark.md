...
## Notes
- Some user stories are missing subtasks
- From your critical path computed, it seems like the path starting with Wav-44 is actually the critical path (especially considering it has a higher priority on Jira)
- WAV-17 is not completed (Status is still in progress)